...
if __name__ == "__main__":
    ...
    output_virtual_image_h5 = (
        Path(checkpoint_dir)
        / marker_folder_name
        / f"{sample_id}-{marker_folder_name}-virtual.h5"
    )
    ...
    if opt.eval:
        model.eval()
    with h5py.File(output_virtual_image_h5, "w") as f:
        f.create_dataset(name="out_shape", data=image.shape)
        # this should be a setting of the inference passed an command line arg
        f.create_dataset(name="overlap", data=np.asarray(overlap))
